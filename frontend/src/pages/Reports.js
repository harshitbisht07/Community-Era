...
const Reports = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Infrastructure Reports</h1>
              ...
              <div className="flex items-start justify-between">
                ...
                <button
                  ...
                  className={`ml-4 flex flex-col items-center px-4 py-2 rounded-lg border-2 transition-colors ${
                    report.voters?.some(v => v._id === user?.id || v === user?.id)
                      ? 'border-primary-600 bg-primary-50 text-primary-600'
                      : 'border-gray-300 hover:border-primary-500 text-gray-700'
                  }`}
                >
                  <FiThumbsUp className="text-xl mb-1" />
                  <span className="text-sm font-semibold">{report.votes || 0}</span>
                </button>
              </div>
            </div>
          ))}
