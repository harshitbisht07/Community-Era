    ...
    <div className="relative h-screen">
      <div className="absolute top-4 left-4 z-[1000] bg-white p-4 rounded-lg shadow-lg max-w-sm">
        <h2 className="text-lg font-semibold mb-2">
          Infrastructure Issues Map
        </h2>
        <p className="text-sm text-gray-600 mb-3">
          {user
            ? "Click on the map to report a new issue"
      ...
      <MapContainer
        center={[28.6139, 77.209]} // Default to a central location (adjust as needed)
        zoom={13}
                  ...
                  <div className="flex items-center justify-between text-xs">
                    <span
                      className={`px-2 py-1 rounded ${getCategoryColor(
                        report.category
                      )}-100 text-${getCategoryColor(report.category)}-800`}
                    >
                      {report.category}
                    </span>
                    <span>
